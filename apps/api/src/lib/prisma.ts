...
/**
 * Tenant-isolation middleware. When a request has an org context, every query on
 * a tenant model is forced to that org — reads/updates/deletes get organizationId
 * added to their `where`, creates get it added to their `data`. findUnique is
 * rewritten to findFirst so the org filter actually applies (a bare unique lookup
 * would otherwise ignore it). With no org context (e.g. pre-login), tenant models
 * are left alone so login can look up the user.
 */
